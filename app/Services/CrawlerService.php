...
class CrawlerService
{
    ...
    public function crawlAllSources(): array
    {
        ...
        echo "{$stats['total_saved']} articles saved from " . (count($this->sources) - count($stats['failed_sources'])) . " successful sources\n";
                ...
                try {
                    ...
                    if ($article) {
                        ...
                        if ($this->storageService->saveArticle($article)) {
                            $stats['saved']++;
                            echo "  ✓ Saved: " . substr($article->title, 0, 70) . (strlen($article->title) > 70 ? "..." : "") . "\n";
                        } else {
    ...
    private function extractArticleData(Crawler $articleNode, array $source): ?Article
    {
        ...
        try {
            ...
            // For The Register specifically, try to find image
            /*if (empty($imageUrl) && $source['name'] === 'The Register') {
                $imageSelectors = [
                    'img', // Any image
                    '[class*="image"]', // Class contains "image"
                    '[class*="img"]', // Class contains "img"
                    '[class*="picture"]', // Class contains "picture"
                    '[class*="thumb"]', // Class contains "thumb"
                    'figure img' // Image inside figure
                ];

                foreach ($imageSelectors as $selector) {
                    $imageElements = $this->filterWithEnhancedSelectors($articleNode, $selector);
                    if ($imageElements->count() > 0) {
                        $imageUrl = $imageElements->attr('src') ?:
                            $imageElements->attr('data-src') ?:
                                $imageElements->attr('data-lazy-src') ?: '';
            ...
            // For The Register specifically, if we're getting empty summaries, try to extract from other elements
            /*if (empty($summary) && $source['name'] === 'The Register') {
                // Try to get a summary from the article's strapline or other elements
                $fallbackSelectors = ['.strapline', '.byline + p', 'p:first-of-type', '.teaser-text'];
                foreach ($fallbackSelectors as $selector) {
                    $fallbackElements = $this->filterWithEnhancedSelectors($articleNode, $selector);
                    if ($fallbackElements->count() > 0) {
                        $summary = trim($fallbackElements->text());
                        $summary = preg_replace('/\s+/', ' ', $summary);
            ...
            if (!empty($selectors['date'])) {
                ...
                if ($dateElements->count() > 0) {
                    ...
                    $dateText = $dateElements->attr('datetime') ?: $dateElements->attr('content') ?: $dateElements->text();
                    ...
                    if ($dateText) {
                        try {
                            ...
                            if ($date) {
                                ...
                                error_log("Could not parse date '{$dateText}' for article '{$title}' from {$source['name']}");
                            }
                        } catch (Exception $e) {
                            // Use current date if parsing fails
                            error_log("Error parsing date '{$dateText}' for article '{$title}' from {$source['name']}: " . $e->getMessage());
                        }
                    }
                }
            }
    ...
    private function fetchFullContent(string $url): string
    {
        try {
            ...
            $contentSelectors = [
                ...
                '.content',
                '.body-content',
                '.article-content', // Added for The Register
                '.page-content'     // Added for The Register
            ];
